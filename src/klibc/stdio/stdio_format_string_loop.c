	...
	while(*format)
	{
		...
		switch(*format)
		{
			...
			case 'b':
			{
				...
				if(flags & FLAGS_SIGNED)
				{
					if(flags & FLAGS_LONG_LONG)
					{
						...
						stdio_print_integer(fp, STDIO_ABS(value), value < 0, base, precision, width,
											flags);
#endif
					}
				...
				else
				{
...
#if STDIO_SUPPORT_LONG_LONG
						stdio_print_integer(
							fp, (stdio_unsigned_value_t)va_arg(args, unsigned long long), flags,
							base, precision, width, flags);
#endif
					}
					else if(flags & FLAGS_LONG)
					{
						stdio_print_integer(fp, (stdio_unsigned_value_t)va_arg(args, unsigned long),
					...
					else
					{
						...
						stdio_print_integer(fp, (stdio_unsigned_value_t)value, false, base,
			...
			case 's':
			{
				...
				{
					size_t l = strnlen(p, precision ? precision : STDIO_MAX_BUFFER_SIZE);
			...
			case 'p':
			{
				...
				{
					stdio_print_integer(fp, (stdio_unsigned_value_t)value, false, BASE_HEX,
										precision, width, flags);
				}
