						 ...
						 size_t precision, size_t width, stdio_flags_t flags)
{
	...
	else
	{
		do
		{
			...
			buffer[len++] = (char)(digit < 10 ? '0' + digit :
												(flags & FLAGS_UPPERCASE ? 'A' : 'a') + digit - 10);
			value /= base;
		} while(value && (len < BUFSIZE));
	}

	stdio_print_integer_finalize(fp, buffer, len, negative, base, precision, width, flags);
}
								  ...
								  stdio_flags_t flags)
{
	...
	if(flags & (FLAGS_HASH | FLAGS_POINTER))
	{
		...
		if((base == BASE_HEX) && !(flags & FLAGS_UPPERCASE) && (len < BUFSIZE))
		{
			buffer[len++] = 'x';
		}
		else if((base == BASE_HEX) && (flags & FLAGS_UPPERCASE) && (len < BUFSIZE))
		{
			buffer[len++] = 'X';
		}
